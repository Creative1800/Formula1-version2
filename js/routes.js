...
import addArticleFormsHandler from "./addarticleFormsHandler.js";
...
function insertArticle (targetElm) {
    let array = [];
    document.getElementById(targetElm).innerHTML = Mustache.render(
        document.getElementById("template-addArticle-form").innerHTML,
        array
    );
    document.getElementById("addArticleForm").addEventListener("submit", event => addArticle(event, targetElm));
}
...
function addArticle(event,targetElm,artIdFromHash,offsetFromHash,totalCountFromHash){
    ...
    .then(responseJSON => {
        const articleId = Number(responseJSON.id);
...
function fetchAndDisplayArticles(targetElm,current){
    ...
    const fetchUrl = `${urlBase}/article/?tag=formula&max=${maxArticles}&offset=${offset}`;
